        ...
        {isLoaded ? 
           <section>
              ...
              <Swiper 
                ...
                className="mySwiper w-[19rem] h-[36rem]"
              >
              {categories.map((category) => (
                <SwiperSlide className=' backdrop-blur border-2 border-solid rounded-bl-lg rounded-tr-lg border-current' key={category.parent}>
                      ...
                      <ul className=" flex gap-2 flex-wrap justify-center tool-list">
                        {toolData
                          .filter((tool) => tool.parent === category.parent)
                          .map((tool, index) => (
                            <li
                              className="flex gap-1 justify-center text-sm items-center  border rounded-bl-lg rounded-tr-lg w-28 h-10 single-tool"
                              key={index}
                            >
                              {tool.name}
                              <img
